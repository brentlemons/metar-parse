...
var conditionsRegEx = /^((\w{3}|\d{3})(\d{2})(?:G(\d{2}))?KT(?:\s(\d{3})V(\d{3}))?)?(?:\s)?((?:(?:(?:[0-9]{1,2})|(?:[0-9]{1,2}\s[0-9]\/[0-9])|(?:[0-9]\/[0-9]))SM))?(.+)(?:\s(?:(?:(M)?(\d{2}))\/(?:(M)?(\d{2}))))(?:\s(?:A(\d{4})))(.*)$/;
    /*
     * m[0] = wind
     * m[1] = direction
     * m[2] = speed
     ...
     * m[6] = visibility
     * m[7] = other?
     * m[8] = temperatureNegative
     * m[9] = temperature
     * m[10] = dewPointNegative
     * m[11] = dewPoint
     * m[12] = altimeter
     * m[13] = remainder
...
class Metar {
    ...
    parseConditions(data) {
        ...
        if (parsed) {
            ...
            var visibility = parsed[7] != undefined ? parsed[7].trim() : undefined;
