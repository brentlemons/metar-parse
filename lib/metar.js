...
var conditionsRegEx = /^((\w{3}|\d{3})(\d{2})(?:G(\d{2}))?KT(?:\s(\d{3})V(\d{3}))?)?(?:\s)?((?:(\d{4})|(?:(?:[0-9]{1,2})|(?:[0-9]{1,2}\s[0-9]\/[0-9])|(?:[0-9]\/[0-9]))SM))?(.+)?(?:\s(?:(?:(M)?(\d{2}))\/(?:(M)?(\d{2}))))(?:\s(?:A|Q(\d{4})))(.*)$/;
    /*
     * m[1] = wind
     * m[2] = direction
     * m[3] = speed
...
class Metar {
    ...
    parseConditions(data) {
        ...
        if (parsed) {
            ...
            var visibility = parseVisibility(parsed[7]);
