...
var parseRegEx = /^(METAR|SPECI)(\sCOR)?\s(\w{4})(\sNIL)?\s([0-9]{6}Z)(\sCOR)?(\sNIL)?(\sAUTO)?(.*)$/;
...
class Metar {
    ...
    constructor(metarString) {
        ...
        if (parsed) {
            ...
            if (parsed[2] != undefined || parsed[8] != undefined) {
                this.correction = true;
            }

            this.station = parsed[3];
            
            this.time = this.parseTimestamp(parsed[5]);
            
            this.auto = parsed[7] != undefined;

            if (parsed[9] != undefined && parsed[9].trim().length > 0) {
                this.conditions = this.parseConditions(this.time, parsed[9].trim(), metarString);
            }
            ...
            console.log('ERROR: Unable to parse METAR -> ' + metarString);
        }
